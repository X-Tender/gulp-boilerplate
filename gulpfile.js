...
gulp.task('single-js', function() {
  return gulp.src([`${jsSrcPath}/**/*.js`,`!${jsSrcPath}/template/**/*js`,`!${jsSrcPath}/bundle/**/*js`])
...
gulp.task('bundle-js', () => {
  return gulp.src(`${jsSrcPath}/bundle/**/*js`)
  .pipe(sourcemaps.init())
...
gulp.task('sentinel-js', done => {
  ...
  var moduleNames = fileList.map( file => {
    return "'"+ file.split('.')[0] + "'";
  }).join(',');

  return gulp.src(`${jsSrcPath}/template/sentinel.template.js`)
    .pipe(replace('\'###MODULES###\'', moduleNames))
    .pipe(sourcemaps.init())
    .pipe(gulpif(productionBuild, minify(options)))
    .pipe(rename(path => {
      path.basename = "sentinels";
    }))
    .pipe(sourcemaps.write('./maps'))
    .pipe(gulp.dest(jsDistPath));
});
...
gulp.task('scss',  () => {
  return gulp.src(`${scssSrcPath}/**/*.scss`)
    ...
    .pipe(gulpif(productionBuild, cssnano({
      filterPlugins: false,
      safe: true,
      mergeRules: false,
      browsers,
...
gulp.task('default', () => {
  gulp.watch([`${jsSrcPath}/**/*.js`,`!${jsSrcPath}/bundle/**/*js`], gulp.series('single-js','sentinel-js') );
